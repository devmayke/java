...
public class Main {
  ...
  public static synchronized void setCadeira(String aNome) {
    System.out.println("** Estava Sentado:" + Cadeira + " Agora:" + aNome);
    Cadeira = aNome;
  }

  public static void main(String[] args) {

    ArrayList<Humano> listaPessoas = new ArrayList<Humano>();

    try {
      listaPessoas.add(new Adulto("Rogério", "123.456.789-01"));
      listaPessoas.add(new Adulto("João", "098.765.432-10"));
      listaPessoas.add(new Adulto("Mariazinha", "777.666.333-22"));
      listaPessoas.add(new Adulto("Mariazinha", "777.666.333-22"));
    } catch (Exception e) {
      e.printStackTrace();
    }
    ...
    for (int i = 0; i < listaPessoas.size(); i++) {
      Thread t1 = new Thread(listaPessoas.get(i));
      t1.start();
    }
